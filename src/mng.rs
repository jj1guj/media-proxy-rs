...
pub(crate) fn is_mng_or_jng(data: &[u8]) -> bool {
	data.starts_with(&SIGNATURE) || data.starts_with(&JNG_SIGNATURE)
}
	...
	while off + 8 <= data.len() {
		...
		match &ctype {
			...
			b"IEND" => {
				...
				cumulative_ticks = cumulative_ticks.saturating_add(ticks);
				let cumulative_ms = cumulative_ticks.saturating_mul(1000) / tps.max(1);
				let ms = cumulative_ms.saturating_sub(emitted_ms);
...
mod tests {
	...
	#[test]
	fn fractional_frame_delays_preserve_total_cycle() {
		let data = build_mng(
			2,
			2,
		...
		let mut data = data;
		data[24..28].copy_from_slice(&3u32.to_be_bytes());
		let anim = decode(&data, 1_000_000, 1000, false).unwrap();
		let cycle_ms: u128 = anim
			.frames
			.iter()
			.map(|frame| {
