...
export RUSTFLAGS="${RUSTFLAGS} -C target-feature=-crt-static -C link-self-contained=no -L native=/vips-system-lib -L native=/vips/lib -C link-arg=-fuse-ld=/usr/bin/mold -C link-arg=-Wl,-rpath-link,/vips-system-lib -C link-arg=-Wl,-rpath-link,/vips/lib -C link-arg=-Wl,--allow-shlib-undefined"
